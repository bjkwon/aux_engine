...
double cay(), cabs();
int lampln(), spln(), zplna(), zplnb(), zplnc(), quadf();
#define fabs(x) ( (x) < 0 ? -(x) : (x) )
#endif

int design_iir(double *num, double *den, int _fs, int _kind, int _type, int _n, double *freqs, double _dbr /*rippledB*/, double _dbd /*stopEdgeFreqORattenDB*/)
{
	//changing the internal variables into the humungous static variables.
	fs = _fs;
	kind = _kind;
	n = _n;
	dbr = _dbr;
...
f2 = freqs[0];
if( (f2 <= 0.0) || (f2 >= fnyq) )	return -5;

if( (type & 1) == 0 )
{
	f1 = freqs[1];
	if( (f1 <= 0.0) || (f1 >= fnyq) ) return -5;
}
else
	f1 = 0.0;
...
if( ((type & 1) == 0) && ((4*n+2) > ARRSIZ) ) return -7;
...
//{
//	int i, type, kind, ord;
//	double *den, *num, freqs[2];
//
//	FILE *fp;
//
//	ord=8;
//	den = calloc(sizeof(double), 2*ord+1);
//	num = calloc(sizeof(double), 2*ord+1);
//
//	freqs[0] = 2000.;
//
//	type=1;
//	kind=1;
//
//	design_iir(num, den, 22050, kind, type, ord, freqs, .5, 0);
//
//	fp=fopen("out.txt","wt");
//	for (i=0; i<ord+1; i++)
//	{
